...
public class VisitRepositoryImpl extends RepositoryImpl<Visit, Long> implements VisitRepository {
	...
	public List<Tuple> getAllVisits() {
		...
		JPAQuery query = jpaFrom(visit).orderBy(visit.year.asc(), visit.month.asc());
		return query.groupBy(visit.month, visit.year).list(visit.intVisited.avg(), visit.month, visit.year);
	}
	...
	public List<Visit> getByFamilyId(Long familyId) {
		...
		query.where(visit.familyId.eq(familyId)).orderBy(visit.year.asc(), visit.month.asc());
