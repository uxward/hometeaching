				...
				<div class="modal-content">
					...
					<div class="modal-body">
						<form id="familyForm">
							...
							<input type="hidden" value="${companion.id}" name="id" /> <input type="hidden" name="visitingTeaching" value="true" />
						</form>
					</div>
		...
		<div class="tab-content">
			<c:forEach var="family" items="${companion.assignments}" varStatus="status">
				<div class="tab-pane ${status.first ? 'active' : ''}" id="${family.id}-tab">
					...
					<c:if test="${canAction}">
						<a href="#recordVisit" role="button" class="btn btn-primary recordVisit" data-assignment-id="${family.assignmentId}" data-family-id="${family.id}" data-toggle="modal">Record visit with the
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					<form id="visitForm">
						...
						<div>
							<input type="hidden" name="visitingTeaching" value="false" /><input type="hidden" name="id" id="visitId" /> <input type="hidden" name="assignmentId" id="assignmentId" /><input type="hidden"
								name="familyId" id="familyId" /><input type="hidden" value="${companion.teachers[0].organization.id}" name="organizationId" />
						</div>
					</form>
				</div>
	...
	<script type="text/javascript">
		...
		function setupVisitHistoryTables() {
			$('.visitHistory').each(function() {
				$(this).dataTable({
					'sDom' : 't'
					,'sAjaxSource' : '<spring:url value="/visit/homeTeaching/"/>?familyId=' + $(this).data('familyId')
					,'aaData' : []
					,'aaSorting': [[ 1, 'desc' ]]
					,'aoColumns' : [ {
						'sTitle' : 'ID',
